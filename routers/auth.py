...
SECRET_KRY = "248a97cc-8638-4d65-a6b3-386e2d8ae31e"
ALGORITHM = "HS256"
...
def create_access_token(username: str, user_id: int, expires_delta: Optional[timedelta] = None):
    ...
    return jwt.encode(encode, SECRET_KRY, algorithm=ALGORITHM)
...
async def create_new_user(create_user: CreateUer, db: Session = Depends(get_db)):
...
async def login_for_access_token(from_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    user = authenticate(from_data.username, from_data.password, db)
...
def get_user_exception():
    excep = HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="could not find user",
                          headers={"WWW-Authenticate": "bearer"})
    return excep


def token_exception():
    excep = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect password or username",
                          headers={"WWW-Authenticate": "bearer"})
    return excep
